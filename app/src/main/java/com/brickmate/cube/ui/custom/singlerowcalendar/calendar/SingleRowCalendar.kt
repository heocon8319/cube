package com.brickmate.cube.ui.custom.singlerowcalendar.calendar
...
import com.brickmate.cube.ui.custom.singlerowcalendar.selection.CalendarDetailsLookup
import com.brickmate.cube.ui.custom.singlerowcalendar.selection.CalendarKeyProvider
...
class SingleRowCalendar(context: Context, attrs: AttributeSet) : RecyclerView(context, attrs) {
    ...
    var initialCurrentDate: Boolean
    ...
    init {
        ...
        context.theme.obtainStyledAttributes(
            ...
            try {
                ...
                initialCurrentDate =
                    getBoolean(R.styleable.SingleRowCalendar_initialCurrentDate, true)
    ...
    fun init() {
        ...
        this.apply {
            ...
            if (!initialCurrentDate) {
                ...
                val posCurrent = getPositionCurrentDate()
                (this.layoutManager as LinearLayoutManager).scrollToPositionWithOffset(
                    posCurrent,
                    0
                )
                setItemsSelected(listOf(posCurrent), true)
            }
    ...
    private fun getPositionCurrentDate(): Int {
        val cur = Calendar.getInstance().time
        val lstDateOfYear = dateList.filter { DateUtils.getYear(it) == DateUtils.getYear(cur) }
        lstDateOfYear.forEach {
            if (DateUtils.getDayNameSimple(it) == DateUtils.getDayNameSimple(cur))
                return dateList.indexOf(it)
        }
        return 0
    }
